...
class ClusterDiscovery:
    def __init__(self, X, feature_count=1000, n_top_words=50, random_state=1):
        """
        :param X: Data points in list of strings form.
        """
        self.X = X
        self.random_state = random_state
        self.feature_count = feature_count
        self.n_top_words = n_top_words
        self.create_vectorizer(self.X)

    def create_vectorizer(self, **vectorizer_settings):
        tfidf_vectorizer = TfidfVectorizer(**vectorizer_settings)
        tfidf_vectorizer = tfidf_vectorizer.fit(self.X)
        self.tfidf_vectorizer = tfidf_vectorizer

    def get_top_features_cluster(self, tf_idf_array, prediction, n_feats, tf_idf_vectorizer):
    ...
    def plot_entropies(self, results_list):
    ...
    def plot_sns(self, dfs, nr_features):
        for i in range(len(dfs)):
            plt.figure(figsize=(8, 6))
            sns.barplot(x='score', y='features', orient='h', data=dfs[i][:nr_features])

    def plot_clusters(self, kmeans, Y_sklearn, use_colors=False):
        if use_colors:
    ...
    def get_entropy_based_best_candidates(self, results_list):
    ...
    def fit(self, X, number_of_topics=10):
        inertias = []
        print("Number of clusters: {}".format(number_of_topics))
        kmeans = KMeans(n_clusters=number_of_topics, max_iter=600, random_state=self.random_state).fit(X)
        # PCA
        dfs, best_features_list = get_top_features_cluster(X, kmeans.predict(Y_sklearn), n_top_words,
                                                           tfidf_vectorizer)
