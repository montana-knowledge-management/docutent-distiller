...
from examples.pu_learning.positive_unlabeled_learning_example import (
    compare_performances,
    estimate_c,
    estimate_positive_count,
    get_potentially_positive_samples,
    inspect_returned_probabilities,
)
...
class MyTestCase(unittest.TestCase):
    ...
    def test_estimate_positive_count(self):
        positive_estimate = estimate_positive_count(
            pu_learner=pu_learner, p_vectors=p_vectors, u_vectors=u_vectors, random_state=1
        )
    ...
    def test_get_potentially_positive_samples(self):
        tp, fp, fn = get_potentially_positive_samples(
            pu_learner=pu_learner, p_vectors=p_vectors, u_vectors=u_vectors, model="lr"
        )
    ...
    def test_compare_performances(self):
        y, yhat_orig, yhat_pu, yhat_best = compare_performances(
            vectorizer=vectorizer,
            pu_learner=pu_learner,
            p_vectors=p_vectors,
            u_vectors=u_vectors,
            c=0.6,
            # c=0.7373,
    ...
    def test_inspect_probabilities(self):
        pos_prob, unlab_prob = inspect_returned_probabilities(
            pu_learner=pu_learner, p_vectors=p_vectors, u_vectors=u_vectors, model="lr"
        )
